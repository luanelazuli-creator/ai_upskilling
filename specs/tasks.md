...
| **TASK-006** | Agente 2: Síntese e Resposta Ancorada | Sintetizador stateless: recebe ContextBundle, produz `SynthesisResult` (com citações inline obrigatórias) ou `NoEvidence` tipado. Validador pós-geração de faithfulness. Modelo `mistral` configurável. | 2026-06-05 | 🧪 Implementado e testado (32 testes em test_spec_006.py; demo `chat_demo.py` funcional com Ollama; wrapper usa `PromptedOutput` por limitação de tool-calling dos modelos locais) |
...
| **TASK-008** | Orquestrador MVP — Pipeline Triagem → Retrieval → ContextBundle → Síntese | Coordenador stateful (1 processo = 1 sessão) que despacha TriageOutput, monta ContextBundle com orçamento de tokens, invoca SynthesisAgent, persiste turno via stub do Agente 3 e expõe CLI REPL. Implementa também `WorkingMemory` (SPEC-MEM §4.1). | 2026-06-05 | 🧪 Implementado e testado (10 testes em test_working_memory.py + 42 em test_spec_008.py; smoke `@pytest.mark.ollama` marcado; REPL `chat.py` substitui `chat_demo.py`) |
